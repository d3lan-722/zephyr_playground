...
Note what is **not** in this tree: the packed register-array that
`api->config()` consumes. That array encodes a specific radar recipe
(antenna count, chirp bandwidth, IF gain, sample rate) and is an
application choice, not a chip property. It lives with the app —
[apps/09_pse84_ai_m33_radar/src/radar_config.h](../apps/09_pse84_ai_m33_radar/src/radar_config.h)
— and is passed *into* the driver at run time.
...
**Register array is app-owned.** The 38-word initialisation sequence
for the sensor lives in the *application*, not the driver:
[apps/09_pse84_ai_m33_radar/src/radar_config.h](../apps/09_pse84_ai_m33_radar/src/radar_config.h).
That file is generated by `bgt60-configurator-cli` from a JSON
description of chirp band, sample count, IF gain, and antenna
selection — all of which are radar-recipe decisions the driver has
no business making. `main.c` passes the array to `api->config()` at
start-up. If you want a different recipe (say, a wider chirp band
for better range resolution), regenerate the header, rebuild the
app; the driver does not change.
