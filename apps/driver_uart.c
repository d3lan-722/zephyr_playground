...
static int ifx_cat1_uart_pm_action(const struct device *dev,
                                   enum pm_device_action action)
{
    const struct ifx_cat1_uart_config *const config = dev->config;
    struct ifx_cat1_uart_data *const data = dev->data;
    /* Build the PDL callback params using the SCB base address and
     * UART context stored in the driver data [4] */
    cy_stc_syspm_callback_params_t cb_params = {
        .base    = (void *)config->reg_addr,
        .context = (void *)&data->context,
    };

    switch (action) {
    case PM_DEVICE_ACTION_SUSPEND:
        /* Ask PDL if UART is ready for Deep Sleep.
         * On success, PDL also disables the UART internally [8] */
        if (Cy_SCB_UART_DeepSleepCallback(&cb_params,
                                          CY_SYSPM_CHECK_READY)
            != CY_SYSPM_SUCCESS) {
            /* TX active or RX FIFO not empty: block low-power entry [7] */
            return -EBUSY;
    ...
    case PM_DEVICE_ACTION_RESUME:
        /* Re-enable UART after wake-up from Deep Sleep [8] */
        if (Cy_SCB_UART_DeepSleepCallback(&cb_params,
...
#define INFINEON_CAT1_UART_INIT(n)                                                                 \
	...
	INTERRUPT_DRIVEN_UART_INIT(n)                                                              \
     /* Define the PM device context for this instance */               \
	...
	static struct ifx_cat1_uart_config ifx_cat1_uart##n##_cfg = {                              \
		...
		IRQ_INFO(n)};                                                                      \
                                                                                           \
      /* Pass PM_DEVICE_DT_INST_GET(n) instead of NULL to register          \
     * the PM context with the device */                               \
    DEVICE_DT_INST_DEFINE(n, &ifx_cat1_uart_init##n,                       \
                          PM_DEVICE_DT_INST_GET(n),                        \
                          &ifx_cat1_uart##n##_data,                        \
                          &ifx_cat1_uart##n##_cfg,                         \
                          PRE_KERNEL_1,                                    \
