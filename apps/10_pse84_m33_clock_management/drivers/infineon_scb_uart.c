...
#define DT_DRV_COMPAT infineon_scb_uart
...
struct ifx_cat1_uart_config {
	...
	const clock_output_t clock_uart;
	const clock_request_t default_clock_request;
	const struct clock_management_data *clk_mgmt;
};
...
/*Clock Management: Request set before baud rate conf.*/
...
static int ifx_cat1_uart_init(const struct device *dev)
{
	...
	ret = clock_management_request_state(config->clk_mgmt,config->default_clock_request);
...
/*
CLOCK_MANAGEMENT_DT_INST_DEFINE(n) defines clock_management_data structure
for the UART instance n.
CLOCK_MANAGEMENT_DT_INST_GET(n) returns pointer to the clock_management_data 
structure for the UART instance n, used to initialize it.
...
{
    const struct ifx_cat1_uart_config *cfg = dev->config;
    return (uint8_t)cfg->clock_uart;
}
...
int ifx_cat1_uart_clk_on(const struct device *dev)
{
    ...
    ret = clock_management_on(cfg->clk_mgmt, cfg->clock_uart);
    if (ret < 0) {
        return ret;
    }
...
int ifx_cat1_uart_clk_off(const struct device *dev)
{
    ...
    ret = clock_management_off(cfg->clk_mgmt, cfg->clock_uart);
    if (ret < 0) {
        return ret;
    }
...
#define INFINEON_CAT1_UART_INIT(n)                                                                 \
	...
	static struct ifx_cat1_uart_config ifx_cat1_uart##n##_cfg = {                              \
		...
		.clock_uart = CLOCK_MANAGEMENT_DT_INST_GET_OUTPUT_BY_NAME(n, uart_clk),            \
		.default_clock_request = CLOCK_MANAGEMENT_DT_INST_GET_REQUEST(n,default),          \
		.clk_mgmt = CLOCK_MANAGEMENT_DT_INST_GET(n),                                       \
		IRQ_INFO(n)};                                                                      \
                                                                                                   \
